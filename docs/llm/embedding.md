...
上面的单句例子只有一层 Token 序列。真实训练为了提高并行效率，通常一次把多句话送入模型，这一组句子称为一个 **batch（批次）**。下面继续使用同一张 5 行、每行 3 维的 Embedding 表，假设一次输入两句话：
...
| 位置 | 读到的 ID | 对应 Token | 执行的查表 | 写入 `hidden` 的值 |
| --- | ---: | --- | --- | --- |
| `[0, 0]` | 1 | 猫 | `E[1]` | `[0.8, 0.1, -0.2]` |
| `[0, 1]` | 2 | 喜欢 | `E[2]` | `[0.2, 0.7, 0.3]` |
| `[0, 2]` | 3 | 鱼 | `E[3]` | `[0.6, -0.1, 0.5]` |
| `[1, 0]` | 4 | 狗 | `E[4]` | `[0.7, 0.2, -0.3]` |
| `[1, 1]` | 2 | 喜欢 | `E[2]` | `[0.2, 0.7, 0.3]` |
| `[1, 2]` | 3 | 鱼 | `E[3]` | `[0.6, -0.1, 0.5]` |

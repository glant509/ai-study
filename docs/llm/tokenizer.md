...
1. **Token、字符、字节和单词有什么区别？**
   - **答案：** 字符是 Unicode 层面的文本元素；字节是字符经过 UTF-8 等编码后的二进制表示；单词是自然语言概念，边界依赖语言；token 是 Tokenizer 根据词表和切分算法生成的模型离散单元。四者不存在固定的一一对应关系。
   - **原文依据：** [Token、字符、单词不是同一个概念](#token、字符、单词不是同一个概念)，以及流程中的[第 1 步：Unicode 字符串](#第-1-步-unicode-字符串——应用程序交出的原始文本)。

2. **BPE 的训练过程与线上编码为什么不同？**
   - **答案：** 训练阶段在大规模语料上统计相邻单元频率，反复合并高频 pair，最终产出固定词表和 merge rules；线上编码不再重新统计语料，而是使用训练完成的规则，把新文本确定性地切成已有 token。
   - **原文依据：** [BPE 的内部逻辑](#bpe-的内部逻辑)和[第 4 步：子词算法](#第-4-步-子词算法——把候选片段拆成词表能够覆盖的单元)。

3. **Normalization、Pre-tokenization 和子词算法分别负责什么？**
   - **答案：** Normalization 统一等价或近似等价的文本表示；Pre-tokenization 根据空格、标点或正则等规则建立初步边界；子词算法再依据词表与合并规则，把候选片段拆成最终 token。
   - **原文依据：** [规范化与预切分](#规范化与预切分)，以及[第 2 步：规范化](#第-2-步-规范化——消除不必要的表示差异)、[第 3 步：预切分](#第-3-步-预切分——先找出允许切分和合并的边界)和[第 4 步：子词算法](#第-4-步-子词算法——把候选片段拆成词表能够覆盖的单元)。

4. **Byte-level BPE 为什么可以避免未登录字符？**
   - **答案：** 任意输入最终都能编码成有限的字节值，基础字节集合可以覆盖任意 Unicode 文本；未见过的字符即使不能作为完整 token 命中，也能退化为若干字节 token，因此不必依赖 `[UNK]` 才能继续编码。
   - **原文依据：** [Byte-level BPE 为什么常见](#byte-level-bpe-为什么常见)。

5. **Chat template 和特殊 token 为什么也占上下文？**
   - **答案：** 角色、消息边界、工具调用边界和结束标记最终都会被序列化并转换为 token ID；模型接收的是包含这些协议 token 的完整序列，所以它们同样消耗上下文窗口和计费 token。
   - **原文依据：** [特殊 Token 与对话模板](#特殊-token-与对话模板)和[第 6 步：特殊 Token](#第-6-步-特殊-token——把协议结构编码进序列)。

6. **为什么不能用字符数代替真实 token 数？**
   - **答案：** 字符到 token 的比例会随模型词表、语言、空格、代码、JSON、Emoji 和特殊 token 改变。容量、限流和费用通常以 token 计算，只有调用目标模型对应的 Tokenizer 才能得到可靠数值。
   - **原文依据：** [Tokenizer 如何影响成本与容量](#tokenizer-如何影响成本与容量)和[动手实验](#动手实验)。

7. **如何为 system、history、RAG、tools 和 output 建立独立预算？**
   - **答案：** 先预留最大输出 token，再从上下文上限中扣除 system、工具 schema 等固定成本；剩余空间分别分配给历史消息、RAG 文档和当前输入，并为每一类设置上限、优先级和超限裁剪策略。
   - **原文依据：** [集中管理 Token Budget](#集中管理-token-budget)和[记录 token 级指标](#记录-token-级指标)。

8. **截断 JSON、Unicode 或工具调用内容有什么风险？**
   - **答案：** 截断可能破坏 JSON 语法、切断 UTF-8/Unicode 组合序列、删除工具调用的参数或边界标记，进而造成解析失败、乱码、schema 校验失败或模型错误理解协议结构。应按完整消息和结构边界裁剪，而不是任意截断字符串或 token 尾部。
   - **原文依据：** [工具调用中的高风险边界](#工具调用中的高风险边界)和[Encode、Decode 与流式输出](#encode、decode-与流式输出)。
